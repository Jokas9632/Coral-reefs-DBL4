...
# Original precision and recall values (class-specific)
...
# Overall metrics (converted to percentages)
resnet_overall_p = 84.48
resnet_overall_r = 77.15

yolo_overall_p = 88.48
yolo_overall_r = 98.35

# Calculate F2 scores for class-specific metrics
...
# Calculate F2 scores for overall metrics
resnet_overall_f2 = calculate_f2(resnet_overall_p, resnet_overall_r)
yolo_overall_f2 = calculate_f2(yolo_overall_p, yolo_overall_r)
...
data = [
    ...
    ['', '', '', ''],  # Separator row
    ['ResNet50\nOverall', f'{resnet_overall_p:.2f}', f'{resnet_overall_r:.2f}', f'{resnet_overall_f2:.2f}'],
    ['YOLOv11\nOverall', f'{yolo_overall_p:.2f}', f'{yolo_overall_r:.2f}', f'{yolo_overall_f2:.2f}']
]
...
colors = ['#f8f9fa', 'white', '#f8f9fa', 'white', '#e9ecef', '#d4e3fc', '#d4e3fc']
for i in range(1, len(data) + 1):
    for j in range(len(columns)):
        ...
        if i == 5:
            cell.set_facecolor('#c5d9f7')
            cell.set_linewidth(2)
        # Bold metric names
        elif j == 0:
            if i >= 6:  # Overall rows
                cell.set_text_props(weight='bold', fontsize=13, ha='left', style='italic')
            else:
...
print(f"\nOverall F2 Scores:")
print(f"ResNet50  - Overall: {resnet_overall_f2:.2f}%")
print(f"YOLOv11   - Overall: {yolo_overall_f2:.2f}%")
